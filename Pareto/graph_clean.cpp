...
Graph::components(const BitSet& DelSet, int *label, int *size) const 
{
  ...
  for(int i=0; i<Nr_nodes; ++i) {
    if( !DelSet.member(i) && label[i]<0 ) {// se il nodo i non � nei nodi cancellati e non fa parte di nessuna componente// 
      S.enqueue(i);
      label[i]=cnt;
...
//cout<<"add_node : "<<i<<", label["<<v<<"] = "<<label[v]<<", size = "<<size[label[v]]<<endl;
  ...
  for(set<int>::iterator si=S.begin(); si!=S.end(); ++si) {
    ...
    new_size+=size[*si];//cio� si parte da 1, il nodo che si sta valutando, + la numerosit� delle attuali componenti
  ...
  return new_size*(new_size-1)/2-old_nr_conn;//  tra tutti nodi che si vogliono inserire, si prende quello per cui l'aumento di connessioni � minimo..
}
...
{ /* Fa il merge delle componenti connesse collegate da i */
  ...
  for(int j=0; j<degree(i); ++j) { // even those present in DelSet ? why do we pass it in argument then ?
    if(!DelSet.member(adj(i)[j]))
    {
      ...
      if( label[v]>=0 && !S.count(label[v]) ) {
        ...
        size[label[v]] = 0;//le altre componenti diventano di dimensone nulla perch� sono unite da i e dai suoi vicini
      }
    }
  }
  ...
  int comp_idx;
  if(new_size > 1)//se ho trovato almeno un vicino di i, devo unire le componenti, senn� no
  {
    comp_idx = *(S.begin());
...
//cout<<"erase li = "<<*li<<endl;
        //if(check.member(*li)) fprintf(stderr,"he ho, %d déjà effacé !!\n",*li);
	N.erase(li);
//cout<<"erased"<<", N.size = "<<N.size()<<endl;
   ...
   if(Maxcp.member(cur)){
    for(int j=0; j<degree(cur); ++j) {
      ...
      if(!C.member(v)) {
	C.add(v);
        //if(check.member(v)) fprintf(stderr,"he ho, %d déjà effacé !! -> %d\n",v,degg[v]);
	N.erase(I[v]);
        //check.add(v);
...
//cout<<"erased"<<", N.size = "<<N.size()<<endl;
    ...
    for(int j=0; j<degree(cur); ++j) { //per tutti i vicini del nodo corrente
      int v=adj(cur)[j];
      //cout<<"v="<<v<<endl;
      if(!C.member(v)) { //se non c'� in C, lo aggiungiamo
	C.add(v);
        //if(check.member(v)) fprintf(stderr,"he ho, %d déjà effacé !! -> %d\n",v,degg[v]);
	N.erase(I[v]);
        //check.add(v);
...
//fprintf(stderr,"cand.size = %d\n", cand.size());
    ...
    if(cand.size() > 0)
    {
      int pos=rand()%cand.size(); //nel caso di pi� candidati: sceglie a caso..
      int cur=cand[pos];
      S->del(cur); //toglie da S, ovvero aggiunge cur al grafo finale
      num--; //un nodo � stato aggiunto
      //fprintf(stderr,"num = %d\n", num);

      if(n == 1 || n == 3)
        C += bestC; // variazione funzione obiettivo
      if(n == 2 && bestC > C)
        C = bestC; // -> senza il test fra bestC e obj, darebbe la dimensione della componente creata dall'introduzione del nodo cur, che non � per forza la pi� grande !
...
//fprintf(stderr,"Can't find a node\n");
  ...
  res.clear();
  for(int i=0; i<NumNodes(); ++i) {//mette i nodi appartenenti a S, cio� nodi da cancellare, nel vettore res..
    if( S->member(i) ) {
      res.push_back(i);
    }
  }
